...
function saveAppearanceSettings() {
    ...
    // Save to localStorage
    localStorage.setItem('petverse_appearance', JSON.stringify(settings));
...
function deleteAccount() {
    if (confirm('¿Estás seguro de que quieres eliminar tu cuenta? Esta acción no se puede deshacer.')) {
        if (confirm('¿Realmente estás seguro? Se perderán todos tus datos, posts y matches.')) {
            ...
            const users = JSON.parse(localStorage.getItem('petverse_users') || '[]');
            const currentUser = getCurrentUser();
            const updatedUsers = users.filter(user => user.email !== currentUser.email);
            localStorage.setItem('petverse_users', JSON.stringify(updatedUsers));
...
function loadSavedSettings() {
    ...
    const appearanceSettings = JSON.parse(localStorage.getItem('petverse_appearance') || '{}');
